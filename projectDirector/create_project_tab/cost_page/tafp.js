 ...
 * Last edited: Bart Kulus 2/10/22
 * 
 */ 
...
function calculateMonths() {  // this function calculates the months needed for a project 
	...
	months = Math.pow(m, (expo)).toFixed(2); // It is PM^1/3 and not PM*1/3 -- I checked the COCOMO model
...
function calculatePersons() {      // this function calculates the # of people needed for a project 
	//persons = (months/PM);
	persons = (PM/months).toFixed(2);  // should be othe rway round -- PM is always greater than months.
	return persons;	
}
...
function personnelCost() {    // calculates the personnel cost of the project 
	...
	personCost = (persons * rate).toFixed(2); 
...
document.getElementById("personnel_cost").textContent = personCost;
document.getElementById("personnel_cost").value = personCost;
...
document.getElementById("personnel_cost").textContent = "";	
}
